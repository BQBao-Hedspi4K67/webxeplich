...
Để đáp ứng nhu cầu thực tế của tổ chức, hệ thống hỗ trợ cơ chế ủy quyền (Delegation) cho phép các quản lý cấp cao tạm thời chuyển giao quyền hạn quản trị cho cán bộ chỉ định mà không cần thay đổi vai trò cơ bản của họ. Đây là giải pháp hybrid giữa phân quyền cơ bản (Role-Based Access Control) và phân quyền chi tiết (Permission-Based Access Control), cho phép linh hoạt xử lý các tình huống như: Quản lý chính vắng mặt, cần nhân viên cấp dưới tạm quản lý danh mục trong thời gian ngắn hạn, hoặc Cấp lãnh đạo ủy thác công việc quản lý lịch trực ban cho cán bộ có năng lực nhưng vẫn giữ vai trò cơ bản là Officer.
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{delegatorUserId:} Người ủy quyền (thường là Admin, Manager hoặc Leader). Đây là người có quyền hạn gốc, muốn giao công việc tạm thời.
...
\begin{enumerate}[leftmargin=1.2cm]
    ...
    \item \textbf{Nâng cấp Quyền hạn Hiệu dụng (Effective Role):} Nếu tìm thấy ủy quyền có hiệu lực, hệ thống nâng cấp quyền hạn của người dùng sang vai trò cao hơn (ví dụ: từ Officer nâng lên Manager, hoặc từ Manager nâng lên Admin). Quyền hạn này được gọi là \texttt{effectiveRole} và sử dụng cho tất cả các quyết định kiểm soát truy cập trong phiên làm việc hiện tại.
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Quản lý Lịch Trực ban:} Kiểm tra cờ \texttt{isDelegatedManager} (hay còn gọi là \texttt{isDelegatedAdmin} nếu được ủy quyền Admin). Nếu bộ cờ này bật, người dùng được phép gán, sửa, xóa ca trực ban cho các cán bộ trong đơn vị, mặc dù vai trò cơ bản của anh ta là Officer.
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Phân quyền Cơ bản (Role-Based):} Vai trò được gán một lần khi tạo tài khoản và ít khi thay đổi. Ví dụ: Officer luôn là Officer.
    \item \textbf{Phân quyền Chi tiết (Permission-Based):} Admin có thể cấp hoặc thu hồi các quyền cụ thể cho từng cán bộ thông qua UC12. Ví dụ: Cấp quyền cho Cán bộ X được quản lý lịch trực ban, nhưng không cho phép Cán bộ Y.
...
\end{itemize}
Điều này giúp Quản trị viên cấp cao có khả năng kiểm toán (Audit) toàn bộ quyết định phân quyền, phát hiện hành vi bất thường (ví dụ: Officer bất ngờ có quyền Admin trong một thời gian), từ đó đảm bảo tính trong sạch và bảo mật của hệ thống quản lý nhân sự.
...
\FloatBarrier
\section{Màn hình 7: Ý kiến trực ban}
