...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Phạm vi chức năng:} Hệ thống tập trung giải quyết các bài toán: Xác thực và quản lý phiên đăng nhập (JWT); Quản lý danh mục cán bộ; Lập và điều chỉnh lịch công tác tuần, lịch trực ban; Tổng hợp \textit{Lịch của tôi} cho từng cá nhân; Gửi và phê duyệt ý kiến trực ban; Hệ thống thông báo nội bộ và Kết xuất dữ liệu (in ấn, xuất file Excel/PDF).
...
\subsection{1.2 Định nghĩa toán học}
...
\subsection{1.3 Thuật toán sử dụng}
...
\subsection{1.5 Phân tích độ phức tạp}
Giả sử:
\begin{itemize}[leftmargin=1.2cm]
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Ban Giám đốc / Lãnh đạo cấp cao:} Là người duyệt lịch công tác tuần chung của toàn Học viện và theo dõi tình hình điều động.
    \item \textbf{Cán bộ Phòng ban chức năng (Quản lý):} Là người trực tiếp tổng hợp danh sách, phân công cán bộ đi trực, phân bổ thời gian hợp lý.
    \item \textbf{Cán bộ, Giảng viên (Officer):} Là người thực thi lịch công tác, lịch trực ban theo phân công.
\end{itemize}
...
Cụ thể, cán bộ quản lý lập một bảng Excel gồm các cột: Thời gian (Từ ngày - Đến ngày), Nội dung công việc, Địa điểm, Người chủ trì, Thành phần tham gia, Ghi chú. Đối với sổ trực ban, thông tin được ghi chép vào một cuốn sổ tay lớn đặt tại phòng trực, yêu cầu người đến ca phải ký nhận.
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item Tự động tính toán và xuất báo cáo thống kê dưới dạng file Excel chuẩn theo yêu cầu của Lãnh đạo chỉ bằng 1 cú click chuột.
...
Biểu đồ chỉ ra sự phân cấp quyền hạn và luồng tương tác rõ rệt giữa 3 nhóm tác nhân:
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Admin (Quản trị viên):} Đóng vai trò là tác nhân có đặc quyền cao nhất. Ngoài việc tương tác với các chức năng nghiệp vụ, Admin là đối tượng duy nhất được phép giao tiếp với Use case \textit{"Quản lý tài khoản"} để cấp phát, chỉnh sửa hoặc thu hồi quyền truy cập của người dùng khác.
    \item \textbf{Manager (Quản lý):} Tập trung vào các Use case mang tính điều hành và phân công nghiệp vụ như \textit{"Lập lịch công tác"}, \textit{"Lập lịch trực ban"} và \textit{"Duyệt ý kiến trực ban"} do cấp dưới gửi lên.
    \item \textbf{Officer (Cán bộ):} Là người dùng cuối (End-user), tương tác chủ yếu với các Use case mang tính chất cá nhân hóa và thực thi nhiệm vụ như xem \textit{"Lịch của tôi"}, tiếp nhận \textit{"Thông báo"} và phản hồi thông qua \textit{"Gửi ý kiến trực ban"}.
\end{itemize}
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item \textbf{Khôi phục phiên (Quan hệ <<extend>>):} Đây là một chức năng mở rộng chạy ngầm. Khi Token sắp hết hạn nhưng người dùng vẫn đang thao tác trên trang, Use case này sẽ tự động kích hoạt để gia hạn phiên làm việc mà không bắt buộc người dùng phải đăng nhập lại, giúp trải nghiệm sử dụng (UX) không bị gián đoạn.
    \item \textbf{Đăng xuất:} Hủy bỏ Token hiện tại, đóng phiên làm việc và đưa người dùng về màn hình khách.
\end{itemize}
...
\textbf{Phân tích chi tiết luồng nghiệp vụ:} 
Quản lý cán bộ là chức năng dành riêng cho Admin và Manager với các quyền hạn:
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Thêm/Sửa/Xóa cán bộ (CRUD):} Cho phép khởi tạo hồ sơ mới, cập nhật chức vụ/đơn vị hoặc xóa/vô hiệu hóa các cán bộ đã chuyển công tác. Hệ thống ràng buộc không cho phép xóa cán bộ nếu họ đang có lịch trực chưa hoàn thành (đảm bảo tính toàn vẹn dữ liệu).
...
\textbf{Phân tích chi tiết luồng nghiệp vụ:} 
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Tạo mới / Cập nhật lịch:} Quản lý thiết lập các trường thông tin: Nội dung công tác, Thời gian, Địa điểm và Thành phần tham gia. 
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Thiết lập lịch trực:} Cho phép Admin/Manager gán cán bộ vào các ca trực cụ thể. Hỗ trợ thêm ghi chú yêu cầu riêng cho từng ca.
...
\textbf{Phân tích chi tiết luồng nghiệp vụ:} 
Thay vì phải dò tìm tên mình trên một tờ lịch giấy dán ở bảng tin, Cán bộ (Officer) tương tác với hệ thống qua các Use case:
\begin{itemize}[leftmargin=1.2cm]
...
\textbf{Mục đích:} Cung cấp kênh giao tiếp chính thức, lưu vết minh bạch để giải quyết các vấn đề phát sinh trong ca trực (báo cáo sự cố, xin đổi ca, đề xuất).
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Phía Cán bộ (Officer):} Khởi tạo luồng bằng cách thực hiện Use case \textit{Gửi ý kiến}. Trạng thái lúc này được hệ thống ghi nhận là Chờ duyệt (Pending).
    \item \textbf{Phía Lãnh đạo/Quản lý:} Tiếp nhận thông qua Use case \textit{Xem danh sách ý kiến}. Sau khi xem xét, quản lý đưa ra quyết định bằng cách gọi Use case \textit{Duyệt} (Approved) hoặc \textit{Từ chối} (Rejected).
    \item \textbf{Ghi chú phản hồi (Quan hệ <<include>>):} Dù Lãnh đạo quyết định Duyệt hay Từ chối, hệ thống đều \textit{bắt buộc} gọi Use case này để yêu cầu nhập lý do/chỉ đạo cụ thể. Điều này giúp Cán bộ hiểu rõ quyết định và lưu lại căn cứ xử lý trên hệ thống.
\end{itemize}

\needspace{0.05\textheight}
\subsection{Nhóm use case Thông báo và Kết xuất dữ liệu}
\textbf{Mục đích:} Kịp thời truyền đạt sự thay đổi của hệ thống đến người dùng và hỗ trợ công tác báo cáo, lưu trữ hồ sơ giấy tờ.

\imgplaceholder{Biểu đồ use case phân rã nhóm thông báo và xuất dữ liệu}{fig:uc_notifications_exports}

\textbf{Phân tích chi tiết luồng nghiệp vụ:} 
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Luồng Thông báo:} Hệ thống cung cấp cơ sở dữ liệu thông báo nội bộ (In-app Notifications). Cán bộ có thể thực hiện \textit{Xem thông báo mới} và \textit{Đánh dấu đã đọc}. Hệ thống sẽ lọc thông báo dựa trên đích đến (gửi cho 1 cá nhân cụ thể hoặc gửi cho toàn bộ người dùng có role là Officer).
    \item \textbf{Luồng Kết xuất dữ liệu (Export):} Chức năng phục vụ cho công tác giao ban, báo cáo định kỳ. Quản lý sử dụng Use case này để trích xuất danh sách lịch theo Tuần/Tháng. Hệ thống hỗ trợ chuyển đổi dữ liệu thành các định dạng tiêu chuẩn (Excel, PDF). Đặc biệt, mọi hành động tải file đều được lưu lại lịch sử (Export Logs) để quản trị viên kiểm soát rủi ro rò rỉ dữ liệu.
\end{itemize}
...
\textbf{Mô tả luồng hoạt động:} 
Luồng bắt đầu từ Officer: Nhập nội dung và Gửi. Trạng thái bản ghi ý kiến lúc này được set là \textit{Pending} (Chờ duyệt) và hệ thống đẩy thông báo tới cấp Quản lý. 
Tiếp theo, Quản lý truy cập danh sách, xem chi tiết và đưa ra 1 trong 2 quyết định (Rẽ nhánh). Dù chọn \textit{Duyệt} hay \textit{Từ chối}, Quản lý đều phải nhập phản hồi. Cuối cùng, hệ thống cập nhật trạng thái ý kiến và gửi thông báo kết quả ngược lại cho Officer.
...
\textbf{Mô tả luồng hoạt động:} 
Người dùng truy cập phân hệ In/Xuất. Chọn tiêu chí (Ví dụ: Xuất lịch trực ban của toàn Học viện trong Tuần 42). Hệ thống truy vấn DB và kết xuất giao diện Preview (Xem trước). Khi người dùng nhấn nút Tải xuống, Backend sẽ sử dụng thư viện để convert (chuyển đổi) dữ liệu JSON thành tệp nhị phân Excel/PDF. File được trả về cho trình duyệt, đồng thời hệ thống ghi lại lịch sử thao tác vào bảng \texttt{export\_logs} để phục vụ công tác thanh tra bảo mật sau này.
...
{Admin (quản trị), Manager/Officer (xem theo quyền)}
{Quản trị danh mục ngày lễ, sự kiện đặc biệt và ngày chào cờ để hiển thị trên lịch tháng.}
{Người dùng đã đăng nhập; Admin có quyền chỉnh sửa dữ liệu ngày lễ.}
...
6b & Hệ thống & User không phải Admin thực hiện thao tác ghi, trả 403. \\ \hline
\end{tabularx}
\end{table}
...
1 & Admin & Mở module Quản lý phòng ban. \\ \hline
2 & Hệ thống & Hiển thị danh sách đơn vị và thống kê nhân sự theo từng đơn vị. \\ \hline
3 & Admin & Tạo mới hoặc sửa tên/loại đơn vị. \\ \hline
4 & Hệ thống & Kiểm tra dữ liệu và cập nhật bảng departments. \\ \hline
5 & Hệ thống & Cập nhật lại các danh sách chọn đơn vị ở form Cán bộ/Tài khoản/Lịch. \\ \hline
\end{tabularx}
\end{table}
...
{Officer (gửi yêu cầu), Admin/Manager (duyệt)}
...
1 & Officer & Chọn ca trực cần xin nghỉ và nhập lý do. \\ \hline
2 & Hệ thống & Lưu yêu cầu với trạng thái Pending. \\ \hline
3 & Hệ thống & Gửi thông báo đến nhóm Admin/Manager phụ trách duyệt. \\ \hline
4 & Admin/Manager & Mở danh sách yêu cầu xin nghỉ, xem chi tiết. \\ \hline
5 & Admin/Manager & Chọn Duyệt hoặc Từ chối và nhập phản hồi. \\ \hline
6 & Hệ thống & Cập nhật trạng thái, lưu người duyệt và thời điểm duyệt. \\ \hline
7 & Hệ thống & Thông báo kết quả về tài khoản Officer tương ứng. \\ \hline
\end{tabularx}
\end{table}
...
\specinfo
{UC12 - Phân quyền lập lịch theo cá nhân}
{Admin}
{Cấp/thu hồi quyền lập lịch trực ban và quyền tạo/duyệt lịch công tác cho từng cán bộ ngoài quyền mặc định theo vai trò.}
{Admin đã đăng nhập; hồ sơ cán bộ đang hoạt động.}
{Quyền thao tác theo cá nhân được cập nhật và có hiệu lực ngay tại giao diện nghiệp vụ.}
{Phân tách rõ quyền theo vai trò (role-based) và quyền cấp riêng (permission-based).}

\mainflowtable{UC12 - Phân quyền lập lịch theo cá nhân}
1 & Admin & Mở tab phân quyền trong module lịch công tác hoặc trực ban. \\ \hline
2 & Hệ thống & Tải danh sách cán bộ cùng trạng thái quyền hiện tại. \\ \hline
3 & Admin & Bật/tắt quyền theo từng cột chức năng (Tạo, Duyệt, Quản lý trực ban). \\ \hline
4 & Hệ thống & Gọi API cập nhật permission theo officerId. \\ \hline
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Cấp Quản trị (Admin):} Truy cập toàn quyền mọi module (Dashboard, Quản lý Cán bộ, Tài khoản, Lập lịch, Tra cứu, Xuất lịch, Quản lý ý kiến).
    \item \textbf{Cấp Quản lý (Manager):} Bị hạn chế chức năng \textit{Quản lý tài khoản hệ thống}; chỉ được phép \textit{Quản lý cán bộ} thuộc đơn vị do mình phụ trách.
    \item \textbf{Cấp Cán bộ (Officer):} Giao diện tối giản, tập trung vào cá nhân: \textit{Dashboard rút gọn, Lịch của tôi, Tra cứu lịch} và \textit{Gửi ý kiến trực ban}.
\end{itemize}
...
\multirow{4}{*}{\textbf{Auth}} & \texttt{POST /auth/login} & Đăng nhập, nhận JWT Token \\ \cline{2-3}
& \texttt{POST /auth/refresh-token} & Cấp lại Token mới \\ \cline{2-3}
...
\hline
\textbf{Nhóm} & \textbf{Method \& Endpoint} & \textbf{Mô tả chức năng} \\ \hline
\multirow{4}{*}{\textbf{Schedules}} & \texttt{GET /work-schedules} & Lấy danh sách lịch công tác chung \\ \cline{2-3}
& \texttt{POST /work-schedules} & Phân công lịch công tác mới \\ \cline{2-3}
& \texttt{GET /work-schedules/my} & Lấy lịch cá nhân của User đang đăng nhập \\ \cline{2-3}
& \texttt{GET /duty-schedules} & Lấy danh sách lịch trực ban \\ \hline
...
\multirow{2}{*}{\textbf{Exports}} & \texttt{GET /exports/schedules} & Xuất lịch ra định dạng Excel/PDF \\ \cline{2-3}
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Quản lý/Admin:} Cung cấp luồng dữ liệu đầu vào bao gồm: Thông tin tài khoản, Hồ sơ cán bộ, Dữ liệu lịch công tác/trực ban, và Quyết định duyệt ý kiến. Hệ thống xử lý và trả về cho họ Báo cáo thống kê, Thông báo hệ thống và Danh sách ý kiến chờ duyệt.
    \item \textbf{Cán bộ (Officer):} Gửi luồng dữ liệu "Ý kiến trực ban" hoặc "Báo cáo sự cố" vào hệ thống. Đổi lại, hệ thống cung cấp luồng thông tin đầu ra là "Lịch của tôi", "Thông báo phân công" và "Kết quả duyệt ý kiến".
\end{itemize}
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item \textbf{Tiến trình 5.0 (Kết xuất dữ liệu):} Tiến trình này chỉ đọc (Read-only) từ kho \textit{D3} và biến đổi thành file báo cáo định dạng Excel/PDF trả cho người dùng.
\end{itemize}
...
\textbf{Phân tích luồng tương tác:}
\begin{enumerate}[leftmargin=1.2cm]
    \item \textbf{Khởi tạo:} Quản lý (Manager) nhập thông tin Form Lịch công tác và bấm Lưu.
    \item \textbf{Gửi Request:} Client gửi yêu cầu `POST /api/work-schedules` kèm JWT Token trong Header lên Server.
    \item \textbf{Xác thực quyền:} Server giải mã Token. Nếu người dùng không phải Admin/Manager, Server từ chối lệnh (`403 Forbidden`).
...
\begin{enumerate}[leftmargin=1.2cm]
    ...
    \item \textbf{Phát sinh thông báo:} Tương tự quy trình tạo lịch, Server tự động tạo một bản ghi Thông báo chứa kết quả duyệt đẩy vào DB để gửi về cho Officer đã khởi tạo ý kiến đó.
...
\textbf{Phân tích luồng tương tác:}
\begin{enumerate}[leftmargin=1.2cm]
    \item \textbf{Khởi tạo:} Người dùng chọn Tiêu chí lọc (VD: Tháng 10) và Format (Excel), sau đó bấm "Xuất báo cáo".
    \item \textbf{Gửi Request:} Client gửi yêu cầu \texttt{GET /api/exports?month=10\&format=excel} lên Server.
    ...
    \item \textbf{Xử lý Buffer/Stream:} DB trả về mảng dữ liệu JSON. Server sử dụng một thư viện (VD: `exceljs` hoặc `pdfkit`) để convert mảng JSON này thành luồng nhị phân (Binary Buffer) của một file Excel/PDF.
...
\textbf{Mục đích:} Biểu đồ Thực thể - Liên kết (Entity-Relationship Diagram - ERD) mô tả cấu trúc dữ liệu vật lý, các thực thể lưu trữ và các ràng buộc toàn vẹn khóa ngoại (Foreign Key) của hệ thống. Cơ sở dữ liệu của dự án đã được thiết kế tối ưu và chuẩn hóa đến dạng chuẩn 3 (3NF - Third Normal Form), loại bỏ hoàn toàn các cột dữ liệu thừa và hiện tượng dị thường dữ liệu.
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \begin{itemize}
        \item Mọi hệ thống an ninh đều cần có cơ chế Audit Trail (Dấu vết kiểm toán). Hai bảng này có mối quan hệ \textbf{1-N} với bảng \texttt{users}. Bất kỳ thao tác Thêm/Sửa/Xóa hoặc hành động xuất dữ liệu (Tải file Excel/PDF) nào cũng sẽ được ghi lại kèm theo \texttt{user\_id} và thời gian thực hiện để truy cứu trách nhiệm khi có sự cố lộ lọt thông tin.
    \end{itemize}
\end{itemize}
...
role & ENUM('leader','manager','officer') & Vai trò nội bộ của cán bộ \\ \hline
...
Hệ thống Backend được tích hợp một module \texttt{activityLogger.js}. Bất kỳ khi nào một Request làm thay đổi dữ liệu (Create, Update, Delete) hoặc liên quan đến bảo mật (Login, Export Excel), module này sẽ âm thầm chạy ngầm để ghi nhận lại dấu vết vào bảng \texttt{activity\_logs} trong CSDL.
...
Hệ thống hiện được chia thành \textbf{14 màn hình chức năng chính}, chi tiết như sau:
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item Nhận và lưu trữ JWT Token vào bộ nhớ cục bộ của trình duyệt. Dựa vào Payload của Token để điều hướng (Redirect) người dùng đến đúng trang Dashboard tương ứng với Role (Admin/Manager/Officer).
    \item Tự động khôi phục phiên đăng nhập nếu phát hiện Token cũ vẫn còn hiệu lực.
\end{itemize}

\subsection{Đánh giá Trải nghiệm người dùng (UX/UI)}
Giao diện được thiết kế tập trung ngay giữa màn hình, loại bỏ mọi chi tiết gây xao nhãng. Hệ thống hỗ trợ thao tác nhấn phím "Enter" để đăng nhập nhanh. Các thông báo lỗi (như "Sai mật khẩu", "Tài khoản bị khóa") được hiển thị trực quan bằng chữ màu đỏ ngay dưới ô nhập liệu để người dùng lập tức nhận biết.
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item Hỗ trợ các thao tác Thêm mới, Cập nhật thông tin và Xóa cán bộ dựa trên phân quyền (Admin/Manager).
...
\subsection{Chi tiết chức năng và Luồng hoạt động}
\begin{itemize}[leftmargin=1.2cm]
    \item Cung cấp công cụ (Form) để khởi tạo Lịch công tác mới với các trường dữ liệu bắt buộc: Tiêu đề, Ngày thực hiện, Khung giờ (Bắt đầu - Kết thúc), Địa điểm, Đơn vị và Người phụ trách.
...
\FloatBarrier
\section{Màn hình 6: Tra cứu lịch}

\subsection{Mục đích nghiệp vụ}
Tạo ra một công cụ tìm kiếm tập trung, mạnh mẽ (Global Search) để truy xuất dữ liệu lịch sử một cách nhanh và chính xác nhất.

\subsection{Chi tiết chức năng và Luồng hoạt động}
\begin{itemize}[leftmargin=1.2cm]
    \item Tra cứu tổng hợp chéo giữa cả hai luồng dữ liệu: Lịch công tác và Lịch trực ban.
    \item Cung cấp bộ lọc tổ hợp: Lọc theo từ khóa nội dung, Lọc theo Trạng thái công việc, Lọc theo Tuần/Tháng hoặc một khoảng ngày (Date Range) tùy chọn.
    \item Cho phép nhấn vào từng dòng kết quả để xem chi tiết (Quick-view) toàn bộ biên bản của lịch đó.
\end{itemize}

\subsection{Đánh giá Trải nghiệm người dùng (UX/UI)}
Giao diện tập trung tối đa diện tích cho bảng kết quả tìm kiếm. Các bộ lọc được xếp gọn gàng ở thanh công cụ phía trên. Thuật toán tìm kiếm áp dụng cơ chế Debounce (Chờ người dùng gõ xong mới tìm) giúp giao diện không bị giật lag và giảm tải cho máy chủ.

\imgplaceholder{Giao diện Tra cứu lịch tổng hợp}{fig:ui_search}

\FloatBarrier
\section{Màn hình 7: Xuất/In lịch}
...
\subsection{Chi tiết chức năng và Luồng hoạt động}
\begin{itemize}[leftmargin=1.2cm]
    \item Cung cấp Form tùy chọn tham số xuất: Chọn nguồn dữ liệu (Lịch công tác/Lịch trực/Tất cả), Chọn phạm vi thời gian (Theo tuần/tháng), Chọn định dạng file (Excel, PDF).
    \item Hiển thị vùng Xem trước dữ liệu (Data Preview) để người dùng kiểm tra thông tin trước khi nhấn tải.
    \item Lưu lại lịch sử các lần xuất file (Export Logs) để quản trị viên có thể theo dõi tính an toàn của thông tin.
\end{itemize}

\subsection{Đánh giá Trải nghiệm người dùng (UX/UI)}
Các nút bấm hành động (Tải Excel, In PDF) được thiết kế nổi bật. Quá trình tạo file trên máy chủ thường mất thời gian, do đó giao diện có tích hợp biểu tượng xoay (Loading Spinner) và vô hiệu hóa nút bấm nhằm thông báo cho người dùng biết hệ thống đang xử lý, tránh việc click liên tục.

\imgplaceholder{Giao diện Xuất báo cáo và In lịch}{fig:ui_export}

\FloatBarrier
\section{Màn hình 8: Ý kiến trực ban}

\subsection{Mục đích nghiệp vụ}
Xây dựng một kênh giao tiếp nội bộ chính thức, thay thế cho việc nhắn tin qua các ứng dụng ngoài, nhằm báo cáo sự cố, đề xuất đổi ca hoặc phản hồi kết quả trực ban.

\subsection{Chi tiết chức năng và Luồng hoạt động}
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Giao diện Cán bộ:} Hiển thị nút "Tạo ý kiến mới", cung cấp trình soạn thảo văn bản để nhập báo cáo. Theo dõi được trạng thái ý kiến của mình đã được lãnh đạo xem hay chưa.
    \item \textbf{Giao diện Quản lý:} Hiển thị danh sách các ý kiến đang chờ duyệt. Cung cấp bộ nút hành động "Phê duyệt" (Approve) hoặc "Từ chối" (Reject). Bắt buộc nhập lý do phản hồi trước khi lưu.
\end{itemize}
...
\imgplaceholder{Giao diện Quản lý Ý kiến trực ban}{fig:ui_opinions}

\FloatBarrier
\section{Màn hình 9: Quy trình chức năng}

\subsection{Mục đích nghiệp vụ}
Đóng vai trò như một sổ tay hướng dẫn sử dụng (User Manual) được nhúng trực tiếp vào phần mềm, giúp chuẩn hóa thao tác và giảm thời gian đào tạo người dùng mới.

\subsection{Chi tiết chức năng và Luồng hoạt động}
\begin{itemize}[leftmargin=1.2cm]
    \item Trình bày tóm tắt sơ đồ luồng công việc của hệ thống.
    \item Giải thích rõ chức năng của từng phân hệ (Tab).
    \item Nêu rõ ràng giới hạn quyền hạn thao tác đối với từng vai trò (Admin, Manager, Officer) để người dùng không bị bỡ ngỡ khi thấy giao diện của mình khác với người khác.
\end{itemize}

\subsection{Đánh giá Trải nghiệm người dùng (UX/UI)}
Sử dụng nghệ thuật chữ (Typography) rõ ràng, chia đoạn hợp lý kết hợp với các biểu tượng (Icons) minh họa giúp trang hướng dẫn không bị nhàm chán và người dùng dễ dàng nắm bắt quy tắc phần mềm chỉ sau vài phút đọc.

\imgplaceholder{Giao diện Hướng dẫn Quy trình chức năng}{fig:ui_workflow}

\FloatBarrier
\section{Màn hình 10: Quản trị tài khoản}

\subsection{Mục đích nghiệp vụ}
Giao diện độc quyền dành cho Quản trị viên (Admin) để thiết lập, cấp phát và thu hồi quyền truy cập hệ thống của nhân sự nội bộ.
...
\imgplaceholder{Giao diện Quản trị tài khoản nội bộ}{fig:ui_account}

\FloatBarrier
\section{Màn hình 11: Quản lý ngày lễ}
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item Admin có quyền Thêm/Sửa/Xóa bản ghi với các thuộc tính: \texttt{holidayDate}, \texttt{holidayName}, \texttt{holidayType}, \texttt{isRecurring}.
...
\FloatBarrier
\section{Màn hình 12: Quản lý phòng ban}
...
\FloatBarrier
\section{Màn hình 13: Quản lý xin nghỉ trực ban}
...
\FloatBarrier
\section{Màn hình 14: Phân quyền lập lịch theo cá nhân}

\subsection{Mục đích nghiệp vụ}
Bổ sung lớp kiểm soát quyền tinh (fine-grained permissions) ngoài mô hình role cơ bản, cho phép Ban Giám đốc điều phối linh hoạt theo nhân sự thực tế.

\subsection{Chi tiết chức năng và Luồng hoạt động}
\begin{itemize}[leftmargin=1.2cm]
    \item Admin bật/tắt quyền lập lịch trực ban theo từng cán bộ trong bảng phân quyền.
    \item Admin bật/tắt độc lập quyền \textit{Tạo lịch công tác} và \textit{Duyệt lịch công tác} theo cá nhân.
    \item Hệ thống đồng bộ ngay trạng thái quyền lên giao diện tác nghiệp của tài khoản tương ứng.
\end{itemize}

\subsection{Đánh giá Trải nghiệm người dùng (UX/UI)}
Thiết kế dạng bảng với thao tác toggle trực tiếp giúp giảm thời gian cấu hình quyền trên tập dữ liệu cán bộ lớn.

\bonusimgplaceholder{Giao diện phân quyền lịch công tác và trực ban}{fig:ui_permission_bonus}
...
\begin{enumerate}[leftmargin=1.2cm]
    ...
    \item \textbf{Kiểm tra xuất dữ liệu:} Sử dụng chức năng Xuất báo cáo, đảm bảo tệp tin Excel/PDF được tải xuống thành công với đúng định dạng và chứa dữ liệu vừa tạo.
\end{enumerate}
...
AUTH-004 & Khi Token hết hạn, hệ thống tự gọi \texttt{/refresh-token}. & Token mới được cấp phát âm thầm, user không bị văng ra ngoài. & \textcolor{green}{\textbf{PASS}} \\ \hline
\end{tabularx}
\end{table}
...
WS-006 & Cán bộ (Officer) cố tình dùng Postman gọi API \texttt{POST /work-schedules}. & Middleware phân quyền phát hiện sai Role, trả lỗi 403 Forbidden. & \textcolor{green}{\textbf{PASS}} \\ \hline
DS-003 & Officer đăng nhập và mở màn hình "Lịch trực của tôi". & Hệ thống chỉ trả về danh sách lịch gắn với ID của Officer đó. & \textcolor{green}{\textbf{PASS}} \\ \hline
\end{tabularx}
\end{table}
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item \textbf{Một số điểm cần làm rõ (Pending):} Kịch bản \textit{Manager duyệt ý kiến của cán bộ thuộc phòng ban khác} cần phải chờ Ban Giám đốc Học viện quy định rõ về cơ chế phê duyệt chéo trước khi cài đặt thuật toán chặn.
\end{itemize}
