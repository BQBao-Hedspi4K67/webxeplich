...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Superadmin (Quản trị hệ thống):} Đóng vai trò là tác nhân có đặc quyền tối cao. Ngoài toàn bộ quyền của Admin, Superadmin là nhóm duy nhất có thể tạo, sửa, xóa tài khoản người dùng và quản lý hồ sơ cán bộ (Master Data). Superadmin không bị ảnh hưởng bởi cơ chế ủy quyền quản lý.
    \item \textbf{Admin (Quản trị viên):} Đóng vai trò là tác nhân có đặc quyền cao nhất sau Superadmin. Ngoài các nghiệp vụ xem và xử lý dữ liệu, Admin có thể quản lý phòng ban, quản lý ngày lễ và cấp/thu hồi các quyền ủy quyền liên quan đến lịch công tác, lịch trực ban. Admin cũng có thể ủy quyền quản lý cho các quản lý cấp dưới.
    \item \textbf{Manager (Quản lý):} Tập trung vào các Use case điều hành nghiệp vụ như \textit{"Lập lịch công tác"}, \textit{"Lập lịch trực ban"}, xem danh sách cán bộ theo phạm vi đơn vị, xử lý đơn xin nghỉ và phê duyệt các lịch chờ duyệt theo quyền được cấp. Manager có thể được ủy quyền để thực hiện các công việc quản lý nâng cấp tạm thời.
    \item \textbf{Officer (Cán bộ):} Là người dùng cuối (End-user), tương tác chủ yếu với các Use case cá nhân hóa như xem \textit{"Lịch của tôi"}, tiếp nhận \textit{"Thông báo"}, gửi \textit{"Đơn xin nghỉ"} và theo dõi trạng thái xử lý. Officer có thể được ủy quyền để quản lý lịch hoặc duyệt lịch trong khoảng thời gian nhất định.
\end{itemize}
...
\textbf{Phân tích chi tiết luồng nghiệp vụ:} 
Quản lý cán bộ là chức năng với phân cấp quyền hạn rõ rệt:
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Thêm/Sửa/Xóa cán bộ (CRUD):} Chỉ Superadmin được phép khởi tạo hồ sơ mới, cập nhật chức vụ/đơn vị hoặc xóa/vô hiệu hóa các cán bộ đã chuyển công tác. Hệ thống ràng buộc không cho phép xóa cán bộ nếu họ đang có lịch trực chưa hoàn thành (đảm bảo tính toàn vẹn dữ liệu). Manager chỉ có quyền xem danh sách theo phạm vi đơn vị của mình.
    \item \textbf{Xem danh sách cán bộ:} Superadmin xem toàn bộ, Admin xem theo phạm vi giao quản lý, Manager xem theo đơn vị trực thuộc, Officer xem danh sách theo quyền được cấp.
    ...
    \item \textbf{Cấp/Thu hồi Quyền Quản lý (Admin):} Admin có thể cấp hoặc thu hồi quyền lập lịch trực ban và quyền tạo/duyệt lịch công tác cho các cán bộ hoặc Manager cấp dưới.
\end{itemize}
...
\begin{itemize}[leftmargin=1.2cm]
    \item \textbf{Phía Cán bộ (Officer):} Chọn ca trực cần xin nghỉ, nhập lý do và gửi đơn. Trạng thái lúc này được hệ thống ghi nhận là Chờ duyệt (Pending).
...
\specinfo
{UC02 - Quản lý cán bộ}
{Superadmin (CRUD), Admin/Manager (xem và ủy quyền theo phạm vi), Officer (xem theo quyền).}
...
{Superadmin được phép tạo/sửa/xóa; Admin/Manager chủ yếu xem, lọc và bật/tắt ủy quyền.}
...
{Admin, Manager}
{Tạo, sửa, xóa, duyệt và theo dõi lịch công tác.}
...
{Officer chỉ có quyền xem dữ liệu theo phạm vi được cấp; lịch chờ duyệt sẽ hiển thị trạng thái pending.}
...
{Hệ thống hỗ trợ lọc theo tuần, cán bộ, loại trực và kiểm tra trạng thái đã tự động phân công.}
...
1 & Officer & Mở module xin nghỉ/ý kiến trực ban. \\ \hline
2 & Officer & Chọn ca trực, nhập lý do và gửi đơn. \\ \hline
3 & Hệ thống & Kiểm tra hợp lệ và lưu yêu cầu ở trạng thái pending. \\ \hline
4 & Hệ thống & Gửi thông báo đến Admin/Manager. \\ \hline
5 & Admin/Manager & Mở danh sách đơn cần xử lý. \\ \hline
...
{Admin (chỉnh sửa), các vai trò khác (chỉ xem danh sách).}
...
3 & Admin & Chọn thao tác thêm mới, sửa hoặc xóa. \\ \hline
...
3 & Admin & Bật/tắt quyền theo từng cột chức năng (Tạo, Duyệt, Quản lý trực ban) hoặc ủy quyền quản trị. \\ \hline
